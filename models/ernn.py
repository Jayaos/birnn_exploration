...
class ERNN(tf.keras.Model):
    def __init__(self, config):
        ...
        self.lstm_bforward = tf.keras.layers.LSTM(2*self.unit_dim, return_sequences=True)
    ...
    def call(self, x, training):
        ...
        x_bforward = self.lstm_bforward(x)
...
def train_ernn(patient_record_path, output_path, vocabsize, test_size, validate_size, epochs, 
    ...
    if pretrained_embedding != None:
        ernn.loadParams(np.load(pretrained_embedding))
    else:
        ernn.initParams()
    ...
    for epoch in range(epochs): 
        ...
        for i in random.sample(range(batch_num), batch_num): # shuffling the data 
            ...
            with tf.GradientTape() as tape:
                    loss = calculate_loss(ernn, x, y, mask)
            gradients = tape.gradient(loss, ernn.trainable_variables)
            ernn.optimizer.apply_gradients(zip(gradients, tbilsternnm.trainable_variables))
        ...
        training_recall = calculate_topk_recall(ernn, train_x, config["vocabsize"], config["batch_size"], k)
        validation_recall = calculate_topk_recall(ernn, valid_x, config["vocabsize"], config["batch_size"], k)
    ...
    save_data(os.path.join(output_path, "training_loss_ernn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.pkl".format(l=num_layers, m=mode, 
    e=epochs, lr=learning_rate, b=batch_size, do=dropout_rate, dim=unit_dim, k=k)), training_loss)
    save_data(os.path.join(output_path, "training_recall_ernn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.pkl".format(l=num_layers, m=mode, 
    e=epochs, lr=learning_rate, b=batch_size, do=dropout_rate, dim=unit_dim, k=k)), training_recall_record)
    save_data(os.path.join(output_path, "validation_recall_ernn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.pkl".format(l=num_layers, m=mode, 
    ...
    ernn.set_weights(best_model)

    print("saving the best model...")
    np.save(os.path.join(output_path, "best_model_ernn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.npy".format(l=num_layers, m=mode, 
    e=epochs, lr=learning_rate, b=batch_size, do=dropout_rate, dim=unit_dim, k=k)), best_model)
    save_data(os.path.join(output_path, "config_ernn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.pkl".format(l=num_layers, m=mode, 
    ...
    avg_topk_recall = calculate_topk_recall(ernn, test_x, config["vocabsize"], config["batch_size"], k)
    print('average top-k recall of the best model on the test set:{r:.6f}'.format(r=avg_topk_recall))
    save_data(os.path.join(output_path, "test_recall_ernn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.pkl".format(l=num_layers, m=mode, 
