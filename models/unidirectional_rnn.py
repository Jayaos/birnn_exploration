...
class UNIRNN(tf.keras.Model):
    def __init__(self, config):
        ...
        self.lstm_forward_layers = [tf.keras.layers.LSTM(self.unit_dim, return_sequences=True) for _ in range(self.num_layers)]
    ...
    def call(self, x, training):
        ...
        for i in range(len(self.lstm_forward_layers)):
            x = self.dropout(x, training=training)
            x = self.lstm_forward_layers[i](x) # (batch_size, max_seq_len, embedding_dim)
        return x
...
def train_unirnn(patient_record_path, output_path, vocabsize, test_size, validate_size, epochs, 
    ...
    unirnn = UNIRNN(config)

    if pretrained_embedding != None:
        unirnn.loadParams(np.load(pretrained_embedding))
    else:
        unirnn.initParams()
    ...
    for epoch in range(epochs): 
        ...
        for i in random.sample(range(batch_num), batch_num): # shuffling the data 
            ...
            with tf.GradientTape() as tape:
                    loss = calculate_loss(unirnn, x, y, mask)
            gradients = tape.gradient(loss, unirnn.trainable_variables)
            unirnn.optimizer.apply_gradients(zip(gradients, unirnn.trainable_variables))
        ...
        training_recall = calculate_topk_recall(unirnn, train_x, config["vocabsize"], config["batch_size"], k)
        validation_recall = calculate_topk_recall(unirnn, valid_x, config["vocabsize"], config["batch_size"], k)
    ...
    save_data(os.path.join(output_path, "training_loss_unirnn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.pkl".format(l=num_layers, m=mode, 
    e=epochs, lr=learning_rate, b=batch_size, do=dropout_rate, dim=unit_dim, k=k)), training_loss)
    save_data(os.path.join(output_path, "training_recall_unirnn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.pkl".format(l=num_layers, m=mode, 
    e=epochs, lr=learning_rate, b=batch_size, do=dropout_rate, dim=unit_dim, k=k)), training_recall_record)
    save_data(os.path.join(output_path, "validation_recall_unirnn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.pkl".format(l=num_layers, m=mode, 
    ...
    unirnn.set_weights(best_model)

    print("saving the best model...")
    np.save(os.path.join(output_path, "best_model_unirnn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.npy".format(l=num_layers, m=mode, 
    e=epochs, lr=learning_rate, b=batch_size, do=dropout_rate, dim=unit_dim, k=k)), best_model)
    save_data(os.path.join(output_path, "config_unirnn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.pkl".format(l=num_layers, m=mode, 
    ...
    avg_topk_recall = calculate_topk_recall(unirnn, test_x, config["vocabsize"], config["batch_size"], k)
    print('average top-k recall of the best model on the test set:{r:.6f}'.format(r=avg_topk_recall))
    save_data(os.path.join(output_path, "test_recall_unirnn{l}_{m}_e{e}_lr{lr}_b{b}_do{do}_d{dim}_k{k}.pkl".format(l=num_layers, m=mode, 
